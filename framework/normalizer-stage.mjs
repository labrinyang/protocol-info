...
// Each normalizer is a default export: normalize({record, evidence, manifest, now}) => {record, changes, gaps}.
// Stage threads record forward, appends each normalizer's changes/gaps (tagged stage='normalize').

import { pathToFileURL } from 'node:url';

export async function runNormalizers({ normalizers, record, evidence, manifest, incomingChanges = [], incomingGaps = [], now = new Date() }) {
  let cur = record;
  ...
  for (const n of normalizers || []) {
    ...
    const out = await fn({ record: cur, evidence, manifest, now });
    cur = out?.record ?? cur;
