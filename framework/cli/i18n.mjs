...
// Usage:
//   node framework/cli/i18n.mjs --manifest M --record R --locales LIST --output-dir D [--parallel N] [--model M] [--fields path,path]
...
const translatableFields = fieldsArg
  ? fieldsArg.split(',').map((s) => s.trim()).filter(Boolean)
  : null;
if (translatableFields) {
  const allowed = new Set(manifest.i18n?.translatable_fields || []);
  const unknown = translatableFields.filter((field) => !allowed.has(field));
  if (unknown.length > 0) {
    console.error(`[i18n] unknown translatable field(s): ${unknown.join(', ')}`);
    process.exit(2);
  }
}
...
const result = await runI18nStage({
  manifest, record, selectedLocales, outputDir, parallelism, modelOverride, translatableFields,
  turnsCap, budgetCap,
